...
def ParticleTracking( Beam, BeamLine, Track_Mode, Delta_s ):
    if Track_Mode == 0:
        ...
        for i in range( NumberOfElements ):
            ...
            ElementsPositions.append( round( ElementsPositions[-1] + BeamLine[i].Length, 10) )
        ...
        if NumberOfElements == 0:
            ...
            for Length in np.arange( Delta_s, round( LengthOfBeamline + Delta_s, 10 ), Delta_s ):
                
                printProgressBar( Length, LengthOfBeamline )
            
                mean_z = np.mean( Beam.BeamMatrix[4,:] )
                RelativisticGammas = np.around( Beam.BeamMatrix[5,:] / Beam.ParticleMass + 1, 10 )
                ...
                for i in range( NumberOfParticles ):
                    for j in range( NumberOfElements ):
                        if Beam.BeamMatrix[4,i] < ElementsPositions[j+1]:
                            ...
                            TransportedParticle[4] = round( TransportedParticle[4] + mean_z + Delta_s, 10 )
...
def BeamTracking( Beam, BeamLine ):
    ...
    for j in range( NumberOfElements ):
        ...
        for i in range( NumberOfParticles ):
            ...
            TransportedParticle = BeamLine[j].Matrix @ tmp_Beam[:,i]
            TransportedParticle[4] = round( TransportedParticle[4] + mean_z + BeamLine[j].Length, 10 )
            TransportedParticle[5] = Beam.BeamMatrix[5,i]
